...
process RUN_LULU {
    ...
    library("lulu")
    matchlist <- read.table("blast_match_list.txt", header=FALSE,as.is=TRUE, stringsAsFactors=FALSE)
    otutab <- read.csv("dada_table_filtered_221102.txt",sep='\t',header=TRUE,as.is=TRUE, row.names = 1)
