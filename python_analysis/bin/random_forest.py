...
def make_predictions(infile, clin_var):
    ...
    data_for_mean['clin_var'] = labels_test
    means_df = data_for_mean.astype(float).groupby('clin_var').mean()
    print(means_df)
    feat_colors = []
    ...
    model = RandomForestClassifier(n_estimators=1000, max_features="sqrt", random_state=44, bootstrap=True, class_weight='balanced')
    #model = BalancedRandomForestClassifier(n_estimators=1000, max_features="sqrt", random_state=44, bootstrap=True, class_weight='balanced')
...
def main():
    ...
    #cancer_feat_importances, cancer_colors = make_predictions(infile, cancer_info)
    ...
    #msi_feat_importances, msi_colors = make_predictions(infile, msi_info)
